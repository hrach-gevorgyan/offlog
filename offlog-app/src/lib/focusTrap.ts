...
// instead of back on the button that opened it.
//
...
export function trapFocus(node: HTMLElement) {
  ...
  if (!node.contains(document.activeElement)) {
    ...
    // panel itself. Focusing "whatever is first in the DOM" would put the
    // caret straight into CardDetail's title textarea on every card open,
    // silently entering edit mode with no intent to do so.
