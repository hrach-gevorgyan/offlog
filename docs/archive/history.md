...
| Release | Summary |
|---|---|
| v6.5.1 | Tooling and documentation only, one user-visible string corrected (the Android widget picker's "your agenda brief" copy, which the widget never rendered). Versioning enforced rather than remembered: `scripts/version.js` writes all three version sources and derives `versionCode`, `version:check` runs in CI and `release.yml` refuses a disagreeing tag. CodeQL's Java/Kotlin analysis moved to its own scoped, cached workflow (4m47s → 3m38s). Documentation rewritten end to end, 29% smaller |
